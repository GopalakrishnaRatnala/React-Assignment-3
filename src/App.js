...
class App extends Component {
  ...
  onClickReset = () => {
    this.setState({activeTabId: tabsList[0].tabId, timer: 60, score: 0})
  }

  matchGame = thumbnailId => {
    const {shuffledIsList, index} = this.state
    if (thumbnailId === shuffledIsList[index].id) {
      this.setState(prevState => ({score: prevState.score + 1}))
    } else {
      this.setState({activeTabId: tabsList[0].tabId, timer: 0})
    }

    this.setState(prevState => ({
      shuffledIsList: prevState.shuffledIsList.sort(() => Math.random() - 0.5),
      index: Math.floor(Math.random() * prevState.shuffledIsList.length),
    }))
  }

  render() {
    const {activeTabId, score, timer, index, shuffledIsList} = this.state
    const filteredThumbNailsList = shuffledIsList.filter(
    ...
    return (
      <div className="app-container">
        ...
        {timer === 0 ? (
          ...
          <div className="match-game-bg-container">
            <img
              src={shuffledIsList[index].imageUrl}
              alt="match"
              className="img-to-select"
            />
            ...
            <ul className="game-item-list-container">
